...
def kernelPCA(d):
    ...
    while(i<1000):
        j=0;
        while(j<1000):
            kmatrix[i][j]=(1+np.dot(file[i],file[j]))**d
            j+=1
    ...
    graph(k_components,d)


def graph(k_components,d):
    if(d==2):
        plt.scatter(k_components[:,0],k_components[:,1])
        plt.title("Projection of points using top 2 PCs for d=2")
        plt.xlabel("Principal Component 1")
        plt.ylabel("Principal Component 2")
        plt.show()
    if(d==3):
        plt.scatter(k_components[:,0],k_components[:,1])
        plt.title("Projection of points using top 2 PCs for d=3")
        plt.xlabel("Principal Component 1")
        plt.ylabel("Principal Component 2")
        plt.show()
